...
_c = ['k','gray'] # way too many colors, trim after
clrs = [c for c in _c]
...
nx.draw_networkx_nodes(G,pos=pos,nodelist=nodes_list1 , node_color='c', label='Stops')
h1 = nx.draw_networkx_edges(G,pos=pos, edgelist =edges_list1 , edge_color = 'k',width = 3)
h2 = nx.draw_networkx(G,pos=pos, edgelist =edges_list2 , edge_color = 'gray', width = 3)
...
plt.savefig("../Results/graph5/lspace_representation.pdf", dpi=150)
plt.show()
...
h1 = nx.draw_networkx_edges(G2,pos=pos, edgelist =edges_list12 , edge_color = 'k',width = 3)
h2 = nx.draw_networkx(G2,pos=pos, edgelist =edges_list22 , edge_color = 'gray', width = 3)
ax1.get_xaxis().set_visible(False)
ax1.get_yaxis().set_visible(False)
proxies = [make_proxy(clr, h2, lw=5) for clr in clrs]
labels = ['Route 1','Route 2']
...
#ax1.legend(handles, labels, loc='center left')
ax1.add_artist(legend)
plt.savefig("../Results/graph5/pspace_representation.pdf", dpi=150)
plt.show()

fig = plt.figure()
ax1 = fig.add_subplot(1, 1, 1)
pos = nx.spring_layout(G3)
nx.draw_networkx_nodes(G3,pos=pos,nodelist=nodes_list13 , node_color='k', label='Stops')
h1 = nx.draw_networkx_edges(G3,pos=pos, edgelist =edges_list13 , edge_color = 'steelblue',width = 3)
#h2 = nx.draw_networkx(G3,pos=pos, edgelist =edges_list22 , edge_color = 'blue', width = 3)
ax1.get_xaxis().set_visible(False)
ax1.get_yaxis().set_visible(False)
clrs2 = ['steelblue']
proxies = [make_proxy(clr, h1, lw=5) for clr in clrs2]
labels = ['Stops']
legend = plt.legend(proxies, labels, loc='upper left')
handles, _ = ax1.get_legend_handles_labels()
labels =['Routes']
ax1.legend(handles, labels, loc='top left')
ax1.add_artist(legend)
plt.savefig("../Results/graph5/cspace_representation.pdf", dpi=150)
